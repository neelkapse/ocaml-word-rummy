...
let sanitize s =
  String.uppercase (String.trim s)
...
let print_game g =
  ...
  let rec print_players ps ws =
    ...
    | (p::pt, w::wt)  ->
      let pts = string_of_int (List.fold_left
                                (fun acc x -> acc + word_value x) 0 p.words) in
      print_string [Bold; yellow] ("\t" ^ p.name ^ " [" ^ pts ^ " points]");
      print_string [] w;
...
let pick_random index =
  let all_names = [["The Voyager"];
                   ["iPhone 4"];
                   ["MacBook Air"];
                   ["Razer Blade"];
                   ["HAL 9000"];
                   ["Deep Thought"]] in
  let names = List.nth all_names index in
...
let create_player_list num_p num_ai ai_diffs =
  ...
  in
  create num_p num_ai [] (List.rev ai_diffs)

let rec input_num_humans () =
  let () = print_string "Enter the number of human players: " in
  try
    let num_players = int_of_string (sanitize (read_line())) in
    if num_players >= 0 then
      num_players
    else
      let () = print_string
...
let rec input_num_ai num_players =
  ...
  try
    let num_ai = int_of_string (sanitize (read_line())) in
...
let rec input_pre_dictionary () =
  ...
  let () = print_string "> " in
  let input = sanitize (read_line ()) in
...
let rec input_ai_diffs num_AI counter =
  if counter > num_AI then
    ...
    try
      ...
      let diff = int_of_string (sanitize (read_line())) in
...
let init () =
  let open ANSITerminal in
  Random.self_init ();
  let () = ignore (Sys.command "clear") in
  print_string [] "\n\n\n\n\n\n\n\n\n\n\n\n\n\n";
...
let draw_turn g (tri_d, hash_d) =
  ...
  print_string ("You must discard one card. Which letter would you like to " ^
                                                                  "discard?\n");
  let discard_letter_string = sanitize (read_line ()) in
...
let rec build_turn g (tri_d, hash_d) =
  let new_word = print_string "Enter the new word you wish to build, or a period [.] to go back:\n> ";
                           string_to_word (sanitize (read_line ())) in
  if new_word = ['.'] then
    g
  else
...
let rec extend_turn g (tri_d, hash_d) =
  ...
  print_string "Enter the word of yours you wish to extend, or a period [.] to go back:\n> ";
  let old_word = string_to_word (sanitize (read_line ())) in
  if old_word = ['.'] then
    g
  else
    let _ = print_string "Enter the new word you wish to form: " in
    let new_word = string_to_word (sanitize (read_line ())) in
...
let rec resolve_steal_conflict candidates =
  ...
  print_string ("> ");
  try
    let input = int_of_string (sanitize (read_line ())) in
    if (input < 1 || input > (List.length candidates)) then
      ...
      List.nth candidates (input - 1)
  with
  | _ -> print_string "That's an invalid number, try again!";
         resolve_steal_conflict candidates

let rec steal_turn g (tri_d, hash_d) =
  print_string ("Enter the word you would like to steal, or a period [.] to go back:\n> ");
  let word = sanitize (read_line ()) in
  if word = "." then
    g
  else
...
let rec check_turn g hash_d =
  ...
  let word_c = string_to_word (sanitize word) in
  ...
  let rec another_input g hash_d =
    ...
    let another = sanitize (read_line ()) in
...
let rec human_turn g (tri_d, hash_d) =
  ...
  else if input = "CLARKSON" then
    AI.play_turn g tri_d 42
  else if input = "SPOCK" then
    AI.play_turn g tri_d 5
  else
    match String.get input 0 with
